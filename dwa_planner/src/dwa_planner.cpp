 #include <algorithm>
        ...
        DWAPlanner::DWAPlanner(void)
            : local_nh_("~"), robot_aligned_(false), scan_updated_(false), robot_reached_(false)
        {   
        ...
        DWAPlanner::Cost::Cost(void) : dist_cost_(0.0), obs_cost_(0.0), velocity_cost_(0.0), total_cost_(0.0) {}

        DWAPlanner::Cost::Cost(const float dist_cost, const float obs_cost, const float direction_cost, const float total_cost)
    ...
    {   
        try
        {    // "map" 좌표계에서 "base_link" 좌표의 변환 정보를 추출
            tf_listener.lookupTransform(goal_.header.frame_id, robot_frame_, ros::Time(0), robot_transform);
             // 로봇의 Pose를 추출 및 대입
    ...
    DWAPlanner::dwa_planner(const Eigen::Vector3d &goal, std::vector<std::pair<std::vector<State>, bool>> &trajectories)
    {
        ...
        const double velocity_resolution = (dynamic_window.max_velocity_ - dynamic_window.min_velocity_) / (velocity_samples_ - 1);
        const double yawrate_resolution = (dynamic_window.max_yawrate_ - dynamic_window.min_yawrate_) / (yawrate_samples_ - 1);
        //유효한 경우의 수 초기화
        ...
        {   // 비용 정규화 및 가중치 적용
            ...
            {          
                if (costs[i].obs_cost_ != 1e6) //유효한 궤적들에 대해서
                {
                    ...
                    costs[i].calc_total_cost();  //정규화 및 가중치가 추가된 최종 비용을 도출
    ...
    geometry_msgs::Twist DWAPlanner::move_robot(void)
    {
        ...
        if (distance_diff > dist_to_goal_th_)
        {
            ...
            if (best_trajectory.first.front().velocity_ == 0.0 && best_trajectory.first.front().yawrate_ == 0.0)
            {
                for (auto &trajectory : trajectories)
                {
                    if (trajectory.first.front().velocity_ != 0.0 || trajectory.first.front().yawrate_ != 0.0)
                    {
  ...
  geometry_msgs::Twist DWAPlanner::align_robot(void)
{
    ...
    if (!robot_reached_)
    {
        if (target_angle_diff > ang_to_goal_th_) // 목표 방향과의 각도 차이가 양수인 경우
        {   
            cmd_vel.linear.x = 0.0;
            cmd_vel.angular.z = 0.2; // 양수 각속도로 회전 (시계 방향)
        }
        else if (target_angle_diff < -ang_to_goal_th_) // 목표 방향과의 각도 차이가 음수인 경우
        {   
            cmd_vel.linear.x = 0.0; 
    ...
    {
        if (goal_angle_diff > ang_to_goal_th_) // 목표 방향과의 각도 차이가 양수인 경우
        {   
            cmd_vel.linear.x = 0.0;
            cmd_vel.angular.z = 0.2; // 양수 각속도로 회전 (시계 방향)
        }
        else if (goal_angle_diff < -ang_to_goal_th_) // 목표 방향과의 각도 차이가 음수인 경우
        {   
            cmd_vel.linear.x = 0.0;
            cmd_vel.angular.z = -0.2; // 음수 각속도로 회전 (반시계 방향)
        }
    ...
    {   
        ...
        for (const auto &cost : costs)
        {
            ...
            {   
                ...
                min_cost.velocity_cost_ = std::min(min_cost.velocity_cost_, cost.velocity_cost_);
                max_cost.velocity_cost_ = std::max(max_cost.velocity_cost_, cost.velocity_cost_);
            }
        }
        ...
        for (auto &cost : costs)
        {
            ...
            {   
                ...
                cost.dist_cost_ = (cost.dist_cost_ - min_cost.dist_cost_) / (max_cost.dist_cost_ - min_cost.dist_cost_ + DBL_EPSILON);
                cost.velocity_cost_ = (cost.velocity_cost_ - min_cost.velocity_cost_) / (max_cost.velocity_cost_ - min_cost.velocity_cost_ + DBL_EPSILON);
            }
        }
    }
    // 궤적의 목표 지점까지의 Cost를 계산하는 함수
    float DWAPlanner::calc_dist_cost(const std::vector<State> &traj, const Eigen::Vector3d &goal)
    {
        ...
        Eigen::Vector2d position_diff = last_position.segment(0, 2) - goal.segment(0, 2);
        return position_diff.norm();
    }



float DWAPlanner::calc_velocity_cost(const std::vector<State> &trajectory, const double prev_yawrate)
{
    ...
    // Calculate the absolute difference between the previous and current yaw rates
...
{   
    ...
    return obs_range_ - min_dist;
     ...
     DWAPlanner::Cost DWAPlanner::evaluate_trajectory(const std::vector<State> &trajectory, const Eigen::Vector3d &goal)
        ...
        void DWAPlanner::state_simulation(State &state, const double velocity, const double yawrate)
        {
...
void DWAPlanner::scan_obstacles(const sensor_msgs::LaserScan &scan)
{
    ...
    for (auto r : scan.ranges)
    {
        ...
        // 스캔 데이터가 없는 영역에 대해서는 r 값을 0.5로 통일해서 지정 -> 미지의 영역으로의 궤적 생성 방지 
